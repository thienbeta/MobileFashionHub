...
const CartSupportScreen = () => {
  ...
  const fetchComboData = useCallback(async () => {
    ...
    try {
      ...
      console.log('Fetching combo data for idCombo:', id);
      const data = await apiFetch(`${API_BASE_URL}/Cart/GioHangByKhachHang?id=${userId}`, 'CartSupport');

      const comboData = data.ctghComboView.find((item: any) => item.idCombo === Number(id));
      console.log('Found comboData:', comboData);
  ...
  const handleRemoveVersion = async (version: number) => {
    ...
    Alert.alert(
      'Xác nhận',
      'Bạn có chắc chắn muốn xóa phiên bản này của combo khỏi giỏ hàng không?',
      [
        { text: 'Hủy', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log('Removing combo version with data:', requestData);
              await apiFetch(`${API_BASE_URL}/Cart/XoaComboVersion`, 'RemoveComboVersion', {
                method: 'DELETE',
                headers: {
              ...
              if (updatedSanPhamList.length === 0) {
                Alert.alert('Thông báo', 'Combo đã bị xóa vì không còn phiên bản nào.');
                setCombo(null);
                router.push('/(tabs)/cart');
              } else {
                const updatedCombo = { ...combo, sanPhamList: updatedSanPhamList };
                setCombo(updatedCombo);
                Alert.alert('Thành công', 'Đã xóa phiên bản combo thành công!');
              }
            } catch (error) {
              console.error('Error removing combo version:', error);
              Alert.alert('Lỗi', 'Không thể xóa phiên bản combo. Vui lòng kiểm tra kết nối hoặc xác thực ngrok.');
            }
          },
        },
      ]
        ...
        renderItem={({ item: versionGroup, index }) => (
          ...
          >
            <TouchableOpacity
              ...
              onPress={() => handleRemoveVersion(versionGroup.version)}
            >
